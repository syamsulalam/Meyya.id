...
async function startServer() {
  ...
  app.get('/api/products', (req, res) => {
    try {
      const products = db.prepare('SELECT p.*, c.name as category_name, c.slug as category_slug FROM products p JOIN categories c ON p.category_id = c.id WHERE p.is_active = 1').all() as any[];
